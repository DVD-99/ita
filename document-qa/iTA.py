...
class Loading_Model():
    def __init__(args_model, args_docs, self):
        ...
        #loading a BART model from hugging Face
    ...
    def get_response(self, q):
        ...
        best_paras = np.argsort(conf) # We sorted the all the paragraphs according to the confidence.
        
        top_para = "question: " + q + " context: " + paras[best_paras[4]] + paras[best_paras[3]] #stores the top 2 ranked paragraphs
        ...
        # Saving the top 2 paragraphs in a file
        top_file = open("/home/bdlabucdenver/Top_para.txt",'w')
        top_file.write(top_para)
        top_file.close()
        # Answer generated by BART Model.
        answer = qa_s2s_generate(top_para, self.bart_model, self.bart_tokenizer,num_answers=1,num_beams=5, min_len=96, max_len=256, max_input_length=1024, device='cuda:0')[0]
